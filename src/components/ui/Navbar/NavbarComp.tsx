...
export default function NavbarComp() {
  ...
  function EditorWithAiButton() {
    return (
      ...
      >
        Editor with AI
        <DiSpark className="w-4 h-4 opacity-0 group-hover:opacity-100 scale-50 group-hover:scale-125 duration-150" />
      </button>
    );
  }
          ...
          <div className="custom-screen-lg gap-x-14 items-center md:flex">
            <div className="flex items-center justify-between py-1 md:block">
              ...
              <div className="flex md:hidden">
                ...
                >
                  {state ? (
                    <FaXmark className="w-5 h-5 pointer-events-none text-zinc-500 group-hover:text-zinc-400" />
                  ) : (
                    <BiArrowBack className="w-5 h-5 pointer-events-none text-zinc-500 group-hover:text-zinc-400" />
                  )}
                </button>
              </div>
            </div>
